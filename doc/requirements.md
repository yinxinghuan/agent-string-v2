...
LEXICON 是一个 `puzzle` 类型的移动端小游戏：语言有重力 — 在滚动的文字流中捕获词语，构建连锁评分机器

## 2. Visual Design

- 整体布局：页面占用 100vw x 100vh，主体验居中，HUD 与操作区覆盖在游戏层上方，移动端以单手操作为优先。
- 背景与配色：主要颜色使用 #FFD700、rgb(50, 140, 70)、rgba(255,255,255,0.95)、#f5f0e6、rgba(80, 100, 180, 0.8)、rgba(248,250,252,0.7)、rgb(60, 140, 80)、#000；高亮元素用于可点击目标、得分、结果或稀有状态。
- 字体：使用 @font、-apple-system, 'Helvetica Neue', sans-serif，按钮与状态文字保持 12-24 px 的可读范围。
...
- `REPEL_F`：5.5
- `COLLECT_R`：140
- `COLLECT_ALPHA_THRESH`：0.35
- `COLLECT_TIME`：0.1
- `TRAP_R`：130
- `TRAP_ALPHA_THRESH`：0.3
- `TRAP_TIME`：0.12
- `REDLINE_Y`：0.50
- `BASE_SCORE_COMMON`：20
- `BASE_SCORE_RARE`：50
